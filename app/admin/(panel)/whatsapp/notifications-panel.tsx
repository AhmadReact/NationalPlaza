...
export function WhatsAppNotificationsPanel({
  ...
  >("");
  const [eventType, setEventType] = useState<WhatsAppEventType | "">("");
  ...
  const [retryNotification] = useRetryWhatsAppNotificationMutation();

  const items = data?.data ?? [];
  const meta = data?.meta;
  const hasActiveFilters = Boolean(search || statusFilter || eventType || orderId);

  const listError = useMemo(() => {
    if (!isError) return null;
    return getFetchErrorMessage(
      error as { status?: number | string; data?: unknown },
      "Failed to load WhatsApp notifications.",
    );
  }, [error, isError]);
  ...
  async function onRetry(id: string) {
    ...
    try {
      ...
      dispatch(
        toast.success(result.message || "Notification retry requested."),
      );
    } catch (err) {
      dispatch(
        toast.error(
          getFetchErrorMessage(
            ...
            "Could not retry the WhatsApp notification.",
          ),
        ),
      );
    } finally {
    ...
    >
      <div className="mb-4 flex flex-col gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {WHATSAPP_NOTIFICATION_STATUSES.map((status) => (
            <button
              key={status}
              type="button"
              onClick={() => {
        ...
        <label className="flex max-w-xs items-center gap-2">
          ...
          <select
            ...
            onChange={(e) => {
              setPage(1);
              setEventType(e.target.value as WhatsAppEventType | "");
            }}
        ...
        {orderId ? (
          <p className="text-xs text-slate-500">
            ...
            <Link href="/admin/whatsapp" className="font-semibold text-brand-800 hover:underline">
              Clear order filter
            </Link>
          </p>
        ) : null}
      ...
      {!isLoading && !listError && items.length > 0 ? (
        <>
          <AdminTable
            columns={["When", "Event", "Order", "Phone", "Status", "Actions"]}
            rows={items.map((item) => {
              const failed = item.status.toUpperCase() === "FAILED";

              return [
                <span key={`${item.id}-when`} className="whitespace-nowrap">
                  {formatDate(item.sentAt || item.createdAt)}
                </span>,
                <div key={`${item.id}-event`}>
                  <p className="font-medium text-brand-950">{item.eventType}</p>
                  {item.templateName ? (
                ...
                item.orderNumber ? (
                  <span
                    ...
                    title={item.orderId ?? item.orderNumber}
                  >
                    {item.orderNumber}
                  </span>
                ) : (
